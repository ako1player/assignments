...
public class BinarySearchTree<T extends Comparable<T>>
    ...
    // for traversals
    protected LinkedList<T> inOrderQueue;    // queue of info
    protected LinkedList<T> preOrderQueue;   // queue of info
    protected LinkedList<T> postOrderQueue;  // queue of info
    ...
    public int size2() throws UnderflowException
    // Returns the number of elements in this BST.
    {
        int count = 0;
        if (root != null) {
            LinkedListStack<BSTNode<T>> hold = new LinkedListStack<BSTNode<T>>();
            BSTNode<T> currNode;
            hold.push(root);
            while (!hold.isEmpty()) {
                currNode = hold.top();
